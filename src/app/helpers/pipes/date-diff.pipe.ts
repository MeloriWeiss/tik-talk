...
export class DateDiffPipe implements PipeTransform {
  ...
  transform(value: string | null | undefined): string {
    ...
    if (days > 0) {
      return DateUtil.createCorrectDateString(':', createdAt.get('hour'), createdAt.get('minute')) + ' ' +
        DateUtil.createCorrectDateString('.', createdAt.get('day'), createdAt.get('month'), createdAt.get('year'));
    }
    ...
    return `${Math.ceil(seconds)} ${DateUtil.getEndOfSecondsBack(seconds)} назад`;
  }
}
