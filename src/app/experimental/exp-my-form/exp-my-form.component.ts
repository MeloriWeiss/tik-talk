...
export class ExpMyFormComponent {
  ...
  form = new FormGroup({
    ...
    phone: new FormControl('', [Validators.required, Validators.minLength(18), Validators.maxLength(18)]),
    ...
    internship: new FormGroup({
      ...
      professions: new FormArray<FormControl<string | null>>([getProfessionControl()], validateRepeatedProfessions)
    })
  });
  ...
  constructor() {
    ...
    this.form.controls.internship.controls.specialization.valueChanges
      .pipe(takeUntilDestroyed())
      .subscribe(value => {
        const currentProfessions = this.internships().find(internship => internship.value === value)?.professions;

        if (currentProfessions) {
          this.currentProfessions = currentProfessions;
        }

        this.form.controls.internship.controls.professions.controls.forEach(profession => {
          profession.reset('');
        })
      });
  }
  ...
  addInternship() {
    if (this.form.controls.internship.controls.professions.length < this.currentProfessions.length) {
      this.form.controls.internship.controls.professions.push(getProfessionControl());
    }
  }
